  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<table>
				<tr>
					...
					<%-- <td><input  name="lastLoginBegin" id="lastLoginStart" onclick="WdatePicker({el:'lastLoginStart',dateFmt:'yyyy-MM-dd',lastLoginStart: '%y-%M-%d'})"  value="<fmt:formatDate value="" pattern="yyyy-MM-dd"/>" type="text" readonly="readonly" style="width:88px;" placeholder="开始日期"  title="最近登录开始"/></td>
					<td><input  name="lastLoginEnd" id="lastLoginEnd" onclick="WdatePicker({el:'lastLoginEnd',dateFmt:'yyyy-MM-dd',lastLoginEnd: '%y-%M-%d'  })"  value="<fmt:formatDate value="" pattern="yyyy-MM-dd"/>" type="text" readonly="readonly" style="width:88px;" placeholder="结束日期"  title="最近登录结束"/></td>
						...
						<c:forEach items="${userList}" var="user" varStatus="vs">
							...
							<tr>
								<td class='center' style="width: 30px;">
									<c:if test="${user.username != 'admin' && user.id != '0'}"><label><input type='checkbox' name='ids' value="${user.id }" id="${user.email }" alt="${user.phone }"/><span class="lbl"></span></label></c:if>
									<c:if test="${user.username == 'admin' || user.id == 0}"><label><input type='checkbox' disabled="disabled" /><span class="lbl"></span></label></c:if>
								</td>
								...
								<td><a title="发送电子邮件" style="text-decoration:none;cursor:pointer;" >${user.email }&nbsp;</a></td>
								</c:if>
								<c:if test="${QX.FX_QX != 1 }">
								<td><a title="您无权发送电子邮件" style="text-decoration:none;cursor:pointer;">${user.email }&nbsp;<%-- <i class="icon-envelope"></i>--%></a></td>
								</c:if>
								<td><fmt:formatDate value="${user.hjsUsersOpt.lastTime}" pattern="yyyy/MM/dd HH:mm:ss" /><br/>
								</td>
								<td>${user.hjsUsersOpt.lastIp}</td>
								<td style="width: 60px;">
									<div class='hidden-phone visible-desktop btn-group'>
										...
										<c:if test="${QX.edit == 1 }">
											<c:if test="${user.username != 'admin' && user.id != '0'}"><a class='btn btn-mini btn-info' title="编辑" onclick="editUser('${user.id }');">编辑</a></c:if>
											<c:if test="${user.username == 'admin' || user.id == '0'}"><a class='btn btn-mini btn-info' title="您不能编辑">编辑</a></c:if>
										</c:if>
										<c:if test="${QX.del == 1 }">
										<c:choose>										
											<c:when test="${user.username=='admin' || user.id == '0'}">
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function sendEmail(EMAIL){
			 ...
			 diag.Drag=true;
			 diag.Title ="发送电子邮件";
			 diag.URL = '${project_name}/head/goSendEmail.do?EMAIL='+EMAIL;
			 diag.Width = 660;
			 diag.Height = 470;
		...
		function sendSms(phone){
			 ...
			 diag.Drag=true;
			 diag.Title ="发送短信";
			 diag.URL = '${project_name}/head/goSendSms.do?PHONE='+phone+'&msg=appuser';
			 diag.Width = 600;
			 diag.Height = 265;
			 diag.CancelEvent = function(){ //关闭事件
				diag.close();
		...
		//新增
		function add(){
			top.mainFrame.showTabPage("用户新增",'${project_name}/user/goAddU.do');
		}
		
		//修改
		function editUser(user_id){
			top.mainFrame.showTabPage("用户修改",'${project_name}/user/goEditU.do?USER_ID='+user_id);
		}
		...
		function delUser(userId,msg){
			bootbox.confirm("确定要删除["+msg+"]吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "${project_name}/user/deleteU.do?id="+userId+"&tm="+new Date().getTime();
