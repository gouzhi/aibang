package com.aibang.framework.utils.ip;
...
public class IPSeeker {
	...
	private static String IP_FILE = Thread.currentThread()
			.getContextClassLoader().getResource("qqwry.dat").getPath();

	//加载配置文件
    static{
		String config_IP_FILE = Const.getProperty("IP.IPSEEKER.IP_FILE");
		if(config_IP_FILE!=null){
			IP_FILE =config_IP_FILE;
		}
    }
	...
	private IPSeeker() {
		...
		b3 = new byte[3];
		try {
			String IP_FILEs = URLDecoder.decode(IP_FILE, "UTF-8");
			ipFile = new RandomAccessFile(IP_FILEs, "r");
	...
	public String getCountry(byte[] ip) {
		...
		/*
		 * if(ipCache.containsKey(ipStr)) { IPLocation loc =
		 * (IPLocation)ipCache.get(ipStr); return loc.country; } else {
		 */
	...
	public String getArea(byte[] ip) {
		...
		/*
		 * if(ipCache.containsKey(ipStr)) { IPLocation loc =
		 * (IPLocation)ipCache.get(ipStr); return loc.area; } else {
		 */
		IPLocation loc = getIPLocation(ip);
		// ipCache.put(ipStr, loc.getCopy());
		return loc.area;
	...
	public String getAddress(String ip) {
		if (ip.startsWith("172.11")) {
			ip = ip.replace("172.11", "192.168");
		}
