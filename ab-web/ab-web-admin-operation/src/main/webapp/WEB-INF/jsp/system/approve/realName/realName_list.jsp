...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
				...
				<table class="center" style="width:50%;">
					<tr height="35">
						<td style="width:150px;" class="center" bgcolor="#438eb9">
							<a href="javascript:void(0);" onclick="goInfoTab('1');" style="text-decoration:none; display:block;"><font color="#fff">身份证认证</font></a>
						</td>
						<td width="5px;"></td>
			...
			<!-- 检索  -->
			<form action="hjsapproverealname/list.do" method="post" name="userForm" id="userForm">
			<table>
				<tr>
					<td>
					...
					<td><input  name="verifyTimeBegin" id="verifyTimeBegin" onclick="WdatePicker({el:'verifyTimeBegin',dateFmt:'yyyy-MM-dd',verifyTimeBegin: '%y-%M-%d'})"  value="<fmt:formatDate value='${checkTimeBegin}' pattern='yyyy-MM-dd' />" type="text" readonly="readonly" style="width:88px;" placeholder="开始日期"  title="开始日期"/></td>
					<td><input  name="verifyTimeEnd" id="verifyTimeEnd" onclick="WdatePicker({el:'verifyTimeEnd',dateFmt:'yyyy-MM-dd',verifyTimeEnd: '%y-%M-%d'  })"  value="<fmt:formatDate value='${checkTimeEnd}' pattern='yyyy-MM-dd' />" type="text" readonly="readonly" style="width:88px;" placeholder="结束日期"  title="结束日期"/></td>
					<c:if test="${QX.cha == 1 }">
			...
			<table id="table_report" class="table table-striped table-bordered table-hover">				
				...
				<c:choose>
					<c:when test="${not empty page.result}">
						<c:if test="${QX.cha == 1 }">
						<c:forEach items="${page.result}" var="RN" varStatus="vs">
									
							<tr>
								<td class='center' style="width: 30px;">${RN.ID}</td>
								<td class='center'>${RN.USERNAME}</td>
								<td class='center'>${RN.REALNAME }</td>
								<td class='center'>${RN.CARD_ID } </td>
								<td class='center'>${RN.VERIFY_USERID }</td>
								<td class='center'>
								<c:if test="${RN.STATUS ==0 }">待审核
								</c:if>
								<c:if test="${RN.STATUS ==1 }">审核通过
								</c:if>
								<c:if test="${RN.STATUS ==-1 }">审核不通过
								</c:if>
								</td>
								<td class='center'><fmt:formatDate value="${RN.VERIFY_TIME }" pattern="yyyy-MM-dd"/></td>
								<td class='center' style="width: 60px;">
									<div class='hidden-phone visible-desktop btn-group'>	
										<c:if test="${QX.edit == 1 }">
											<a class='btn btn-mini btn-info' onclick="auditUser('${RN.ID}');" title="审核">审核</a>
										</c:if>
									</div>
								</td>
							</tr>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function auditUser(id){
			top.jzts();
			 ...
			 diag.Drag=true;
			 diag.Title ="实名认证";
			 diag.URL = '${project_name}/hjsapproverealname/toaudit.do?id='+id;
			 diag.Width = 600;
			 diag.Height = 270;
			 diag.CancelEvent = function(){ //关闭事件		
				top.jzts(); 
				setTimeout("self.location.reload()",100);
				top.mainFrame.reloadPage(/hjsapproverealname\/list.do/);
				diag.close();
			 };
			 diag.show();
		}
