    ...
    <div class="account-Right fl">
		...
		<div class="whiteR329">
        	<div class="receivablesTab">
            	<span onclick = "havingDetail()">待收款明细</span>
                <span onclick = "overDetail()" class="mingxi">已结请明细</span>
            </div>

            <form id = "overDetail" action="${project_name}/user/invest/receiving_over_detail" method="post">
                ...
                <div class="receivablesTable" style="display:block;">
                	<table class="tableHead">
                    	<tr>
                        	<th class="receSp1">收款时间</th>
                            <th class="receSp2">投资名称</th>
                            <th class="receSp3">收款（元）</th>
                            <th class="receSp4">本金（元）</th>
                            <th class="receSp5">收益（元）</th>
                    ...
                    <tr class="tableList">
                        	<td><fmt:formatDate value="${item.RECOVER_YESTIME }" pattern="yyyy-MM-dd "/></td>
                            <td class="receSp2"><span>${item.B_NAME }</span></td>
                            <td>${item.APPR+item.TENDER_ACCOUNT }</td>
                            <td>${item.TENDER_ACCOUNT }</td>
                            <td>${item.APPR }</td>
                            <td class="receSp6"><a href="user/invest/investOverDetail?tId=${item.tId}">[ 详情 ]</a></td>
                    </tr>
              	</c:forEach>
              	</c:when>
              	...
              	<c:when test="${page.totalCount==0 && page.pageNumber==1 }">	<tr><td style="text-align:center;height: 150px;line-height: 150px;" colspan='6'>暂无记录! </td></tr></c:when>
					...
					<p class="number">
						<span>共<i>${count }</i>笔，收款额：<i>${receivTotal }</i>元</span>
						<span>本金：<i>${AccountTotal }</i>元</span>
						<span>收益：<i>${total }</i>元</span>
					</p>
