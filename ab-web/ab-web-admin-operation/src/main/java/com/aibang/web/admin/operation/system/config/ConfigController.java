...
import com.aibang.business.api.system.config.AbSysConfigService;
...
public class ConfigController extends AdminController{
	...
	private AbSysConfigService hjsSysConfigService;
	...
	public String save(ModelMap model,AbSysConfig hsd ,PrintWriter out,HttpServletRequest request) throws Exception{
		...
		if(null != PARENT_ID && "0".equals(PARENT_ID)){
			...
			AbSysConfig pdp = hjsSysConfigService.getById(hsd.getParentId());
	...
	public String edit(ModelMap model,AbSysConfig hsd ,PrintWriter out,HttpServletRequest request) throws Exception{
		String PARENT_ID = hsd.getParentId()+"";	
		//判断父类ID是否为空 进行不同的处理
		if(null != PARENT_ID && "0".equals(PARENT_ID)){
			hsd.setLayer(1);
	...
	@RequestMapping
	public String list(ModelMap model,AbSysConfigQuery hsdq)throws Exception{
		...
		if(null != hsdq.getParentId() && !"".equals(hsdq.getParentId()+"") && !"0".equals(hsdq.getParentId()+"")){				 
			...
			//头部导航
			szdList = new ArrayList<AbSysConfig>();
	...
	public void getZDname(String PARENT_ID) {
		logBefore(logger, "递归");
		try {
			AbSysConfig hsd = hjsSysConfigService.getById(Integer.parseInt(PARENT_ID));
			if(hsd != null){
				szdList.add(hsd);
	...
	public String toAdd(ModelMap model,AbSysConfig pd){
		try{
	...
	public String toEdit( ModelMap model,AbSysConfigQuery hsdq )throws Exception{
		AbSysConfig pd = hjsSysConfigService.getById(hsdq.getId());
	...
	public void has(AbSysConfigQuery hsdq,PrintWriter out){
	...
	@ResponseBody
	public Object del(AbSysConfig hsd){
		Map<String,String> map = new HashMap<String,String>();
		String errInfo = "";
		try{
			AbSysConfigQuery hsdq = new AbSysConfigQuery();
