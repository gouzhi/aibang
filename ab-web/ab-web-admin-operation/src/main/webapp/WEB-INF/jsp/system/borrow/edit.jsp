...
$(function(){
	top.mainFrame.reloadPage(/borrow\/unOnline_page/);
});
</script>
<body>
<div class="center centerW">
<form id="productForm" method="post" action="borrow/update.do">
	...
	<div class="row-fluid">
	    ...
	    <div class="span4"><input type="text" placeholder="请输入产品名称" class="span11" name="bname" value="${borrow.bname }" validate="{required:true,remote:'borrow/isBnameExist?borrowid=${borrow.id}',messages:{required:'请填写产品名称',remote:'产品名称已经存在'}}"/><br/></div>
	    ...
	    <div class="span4">
	        <select class="span11" name="repayType" id="repayType" validate="{required:true,messages:{required:'请选择还款周期'}}">
	           ...
	           <option value="${item.code }" <c:if test="${borrow.repayType eq item.code}">selected="selected"</c:if>>${item.cname }</option>
	           </c:forEach>
	       </select><br />
	    </div>
	</div>
         ...
         <div class="span4">
         	<select class="span11" name="projectid" id="projectid" validate="{required:true,messages:{required:'请选择关联项目'}}">
         		...
         		<c:forEach items="${projects }" var="item">
         			<option value="${item.id }" <c:if test="${borrowUser.projectId eq item.id}">selected="selected"</c:if>>${item.pname }</option>
         		</c:forEach>
            </select><br />
         </div>
         <div class="span2 pl"><span class="required">*</span>开始时间：</div>
         <div class="span4">
             <input class="Wdate span11" type="text" placeholder="点击显示日历" onclick="var date2=$dp.$('tenderEndTime');WdatePicker({onpicked:function(){tenderEndTime.click();tenderEndTime.focus();},minDate:'%y-%M-{%d}',dateFmt:'yyyy-MM-dd'})" id="tenderStartTime" name="tenderStartTimeString" validate="{required:true,messages:{required:'请选择开始时间'}}" value="<fmt:formatDate value="${borrow.tenderStartTime }" pattern="yyyy-MM-dd"/>"/><br/>
    	</div>
     </div>
     <div class="row-fluid">
         ...
         <div class="span4">
             <select class="span11" name="borrowType" validate="{required:true,messages:{required:'请选择产品类型'}}">
	           ...
	           <option value="${item.code }"  <c:if test="${borrow.borrowType eq item.code}">selected="selected"</c:if>>${item.cname }</option>
	           </c:forEach>
            </select><br />
         </div>
          <div class="span2 pl"><span class="required">*</span>结束时间：</div>
    	<div class="span4">
        	<input class="Wdate span11" type="text" placeholder="点击显示日历" onclick="WdatePicker({minDate:'#F{$dp.$D(\'tenderStartTime\',{d:1});}',dateFmt:'yyyy-MM-dd'})" id="tenderEndTime" name="tenderEndTimeString" validate="{required:true,messages:{required:'请选择结束时间'}}" value="<fmt:formatDate value="${borrow.tenderEndTime }" pattern="yyyy-MM-dd"/>"/><br/>
    	</div>
     </div>
     <div class="row-fluid">
     	<div class="span2 pl"><span class="required">*</span>年化收益：</div>
         <div class="span4">
             <input type="text" readonly="readonly"  placeholder="请输入年化收益" class="span11" name="borrowApr" value="${borrow.borrowApr }" validate="{required:true,isDigitsDot:true,messages:{required:'请输入年化收益'}}" /><br/>
             <b>%</b>
         </div>
        <div class="span2 pl"><span class="required">*</span>最低投资金额：</div>
    	<div class="span4"><input type="text" placeholder="请输入最低投资金额"  maxlength="8" class="span11" name="tenderAccountMin" value="<fmt:formatNumber value="${borrow.tenderAccountMin }" pattern="#"/>" id="tenderAccountMin" validate="{required:true,isIntGtZero:true,maxlength:8,digits:true,messages:{required:'请输入起投金额'}}"/><br/><b>元</b></div>
     </div>
     <div class="row-fluid">
     	<div class="span2 pl"><span class="required">*</span>借款总额：</div>
         <div class="span4">
             <input type="text" placeholder="请输入资金规模" class="span11"  maxlength="8" name="account" value="<fmt:formatNumber value="${borrow.account }" pattern="#"/>" id="account" validate="{required:true,isIntGtZero:true,maxlength:8,digits:true,maxlength:8,messages:{required:'请输入借款总额'}}"/><br/>
             <b>元</b>
         </div>
    	<div class="span2 pl"><span class="required">*</span>递增金额：</div>
    <div class="span4"><input type="text" placeholder="请输入递增金额" maxlength="8" class="span11" name="tenderAccountStep" value="<fmt:formatNumber value="${borrow.tenderAccountStep }" pattern="#"/>" validate="{required:true,isIntGtZero:true,maxlength:8,digits:true,messages:{required:'请输入递增金额'}}"/><br/><b>元</b></div>
	</div>
<div class="row-fluid">
	<div class="span2 pl"><span class="required">*</span>投资周期：</div>
    <div class="span4">
        <input type="text" placeholder="请输入投资周期" class="span11" maxlength="8" name="borrowPeriod" value="${borrow.borrowPeriod }" validate="{required:true,isIntGtZero:true,maxlength:8,digits:true,messages:{required:'请输入投资周期'}}"/><br/>
        <b id="borrowPeriodType">月</b>
    </div>
    <div class="span2 pl"><span class="required">*</span>最高投资金额：</div>
    <div class="span4"><input type="text" placeholder="请输入最高投资金额" maxlength="8" class="span11" name="tenderAccountMax" value="<fmt:formatNumber value="${borrow.tenderAccountMax }" pattern="#"/>" id="tenderAccountMax" validate="{required:true,isIntGtZero:true,maxlength:8,digits:true,messages:{required:'请输入最高贷款金额'}}"/><br/><b>元</b></div>
</div>
<div class="row-fluid">
...
<div class="row-fluid">
    <div class="span11 pL40">
        ...
        <textarea rows="" cols="" id="porjectRemark" name="porjectRemark">${borrow.porjectRemark}</textarea>
    </div>
</div>
<div class="row-fluid">
...
<div class="row-fluid">
    <div class="span11 pL40">
        <!--<script id="borrowUserRemark" name="borrowUserRemark" type="text/plain">${borrow.borrowUserRemark}</script>  -->
         <textarea rows="" cols="" id="borrowUserRemark" name="borrowUserRemark">${borrow.borrowUserRemark}</textarea>
    </div>
</div>
<%-- <div class="row-fluid">
    ...
    <div class="span4">
     	<select class="span11" name="vouchId" id="vouchId" validate="{required:true,messages:{required:'请选择关联项目'}}">
     		...
     		<c:forEach items="${enterprices }" var="item">
     			<option value="${item.id }" <c:if test="${borrow.vouchId eq item.id}">selected="selected"</c:if>>${item.entName }</option>
     		</c:forEach>
        </select><br />
     </div>
 </div> --%>
...
<div class="row-fluid">
    <div class="span11 pL40">
        ...
        <textarea rows="" cols="" id="vouchRemark" name="vouchRemark">${borrow.vouchRemark}</textarea>
    </div>
</div>
<div class="row-fluid">
...
<div class="row-fluid">
    <div class="span11 pL40">
        ...
        <textarea rows="" cols="" id="windControl" name="windControl">${borrow.windControl}</textarea>
    </div>
</div>
<!-- <div class="row-fluid">
...
<%-- <div class="row-fluid">
    ...
    <div class="span2"><a href="#"  style="disay:block;height:75px;line-height:75px;">点击查看</a></div>
    <div class="span6 marTB">
        <form>
            ...
            <input type="hidden" value="1" name="isUploadFile" id="isUploadFile" validate="{required:true,messages:{required:'请上传word文件'}}">
        </form>
         <p>word模板另存为html格式</p>
    </div>
    </div> --%>
