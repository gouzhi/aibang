...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
				...
				<table class="center" style="width:50%;">
					<tr height="35">
						...
						<td style="width:150px;" class="center" bgcolor="#438eb9">
							<a href="javascript:void(0);" onclick="goInfoTab('3');" style="text-decoration:none; display:block;"><font color="#fff">邮箱认证</font></a>
						</td>
						<td width="5px;"></td>
						<td style="width:150px;" class="center" bgcolor="#E5E5E5">
			...
			<!-- 检索  -->
			<form action="hjsapproveemailactive/list.do" method="post" name="userForm" id="userForm">
			<table>
				<tr>
					<td>
			...
			<table id="table_report" class="table table-striped table-bordered table-hover">				
				...
				<c:choose>
					<c:when test="${not empty page.result}">
						<c:if test="${QX.cha == 1 }">
						<c:forEach items="${page.result}" var="EM" varStatus="vs">
									
							<tr>
								
								<td class='center' style="width: 30px;">${EM.ID}</td>
								<td class='center'>${EM.USERNAME}</td>
								<td class='center'>${EM.EMAIL }</td>
								<td class='center'>
								<c:if test="${EM.STATUS ==0 }">待审核
								</c:if>
								<c:if test="${EM.STATUS ==1 }">审核通过
								</c:if>
								<c:if test="${EM.STATUS ==-1 }">审核不通过
								</c:if>
								</td>
								<td class='center'><fmt:formatDate value="${EM.CREATE_TIME }" pattern="yyyy-MM-dd"/></td>
								<td class='center' style="width: 60px;">
									<div class='hidden-phone visible-desktop btn-group'>	
										<c:if test="${QX.edit == 1 }">
											<a onclick="auditUser('${EM.ID}');" class='btn btn-mini btn-info' title="审核" >审核</a>
										</c:if>
									</div>
								</td>
							</tr>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function auditUser(id){
			top.jzts();
			 ...
			 diag.Drag=true;
			 diag.Title ="邮箱认证";
			 diag.URL = '${project_name}/hjsapproveemailactive/toaudit.do?id='+id;
			 diag.Width = 600;
			 diag.Height = 270;
