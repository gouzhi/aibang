...
<body>
	<form action="hjsactinfo/edit" name="Form" id="Form" method="post">
		...
		<div class="row-fluid">
		    ...
		    <div class="span3"><input type="text" placeholder="请输入活动名称" class="span11" name="aname" id="aname" validate="{required:true,messages:{required:'请输入活动名称'}}" maxlength="50" value="${actInfo.aname}"/></div>
    	 ...
    	 <div class="span2 pl"><span class="required">*</span>结束时间：</div>
    		<div class="span3">
        	<input class="Wdate span11" type="text"  onclick="WdatePicker({minDate:'#F{$dp.$D(\'startTime\',{d:0});}',dateFmt:'yyyy-MM-dd'})" id="endTime" name="stopTimeString" validate="{required:true,messages:{required:'请选择结束时间'}}" value="<fmt:formatDate value="${actInfo.stopTime }" pattern="yyyy-MM-dd"/>"/>
    	 </div>
    	 <div class="span2 pl"><span class="required">*</span>活动预算：</div>
		    <div class="span3"><input type="text" placeholder="请输入活动预算" class="span11" name="budgetTotal" id="budgetTotal" validate="{required:true,messages:{required:'请输入活动预算'}}" value="${actInfo.budgetTotal}"/></div>
    	</div>
     <div class="row-fluid">
		    <div class="span2 pl"><span class="required">*</span>活动规则：</div>
		    <div class="span3"><input type="text" placeholder="请输入活动规则" class="span11" name="actRules" id="actRules" validate="{required:true,messages:{required:'请输入活动规则'}}" maxlength="100" value="${actInfo.actRules}"/></div>
	 		 <div class="span2 pl"><span class="required">*</span>活动链接：</div>
		    <div class="span3"><input type="text" placeholder="请输入活动链接" class="span11" name="actUrl" id="actUrl" validate="{required:true,messages:{required:'请输入活动链接'}}" maxlength="100" value="${actInfo.actUrl}"/></div>
	 </div>
    <div class="row-fluid">
	 ...
	 <div class="row-fluid" id="use_type">
		    ...
		    <div class="span3"  >
		    	  <input type="checkbox" placeholder="Web"  name="systemScope" class="systemScope"  
			     value="1" <c:if test="${fn:contains(actInfo.systemScope,'1')}">checked="checked"</c:if> style="width: 16px;"/><span class="lbl"></span>&nbsp;&nbsp;Web&nbsp;&nbsp;
			    <input type="checkbox" placeholder="Android"  name="systemScope" class="systemScope"  
			     value="2" <c:if test="${fn:contains(actInfo.systemScope,'2')}">checked="checked"</c:if>  style="width: 16px;line-height: 16px;"/><span class="lbl"></span>&nbsp;&nbsp;Android&nbsp;&nbsp;
			    <input type="checkbox" placeholder="Ios"  name="systemScope" class="systemScope" 
			     value="3" <c:if test="${fn:contains(actInfo.systemScope,'3')}">checked="checked"</c:if>  style="width: 16px;"/><span class="lbl"></span>&nbsp;&nbsp;Ios&nbsp;&nbsp;
			    <input type="checkbox" placeholder="H5"  name="systemScope" class="systemScope" 
			     value="4" <c:if test="${fn:contains(actInfo.systemScope,'4')}">checked="checked"</c:if>  style="width: 16px;line-height: 16px;"/><span class="lbl"></span>&nbsp;&nbsp;H5&nbsp;&nbsp;
		    </div>
	</div>	
