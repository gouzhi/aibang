...
<div class="row-fluid">
				...
				<tr>
					...
					<td>
						<div class='hidden-phone visible-desktop btn-group'>
							...
							<c:if test="${user.USERNAME == 'admin'}"><a class='btn btn-mini btn-info' title="不能编辑">编辑</a></c:if>
							</c:if>
							<c:choose>
		...
		<table style="width:100%;">
			<tr>
				<td style="vertical-align:top;">
					...
					<c:if test="${QX.del == 1 }">
					<a class="btn btn-small btn-danger" onclick="delAll();" title="批量删除" >删除</a>
					</c:if>
				</td>
			</tr>
		</table>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		<script type="text/javascript" src="${css_imagedomain}/js/jquery.dataTables.min.js"></script>
		<script type="text/javascript" src="${css_imagedomain}/js/jquery.dataTables.bootstrap.js"></script>
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '${project_name}/user/goAddU.do';
			 diag.Width = 225;
			 diag.Height = 415;
		...
		function editUser(user_id){
			 ...
			 diag.Drag=true;
			 diag.Title ="资料";
			 diag.URL = '${project_name}/user/goEditU.do?USER_ID='+user_id;
			 diag.Width = 225;
			 diag.Height = 415;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function delUser(userId,msg){
			bootbox.confirm("确定要删除["+msg+"]吗?", function(result) {
				if(result) {
					var url = "${project_name}/user/deleteU.do?USER_ID="+userId+"&tm="+new Date().getTime();
