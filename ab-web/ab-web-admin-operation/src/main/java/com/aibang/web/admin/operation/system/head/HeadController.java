package com.aibang.web.admin.operation.system.head;
	...
	public Object sendSms(){
		...
		if("yes".endsWith(isAll)){
			try {
				...
				userList = "appuser".equals(fmsg) ? appuserService.listAllUser(new User()):userService.listAllUser(new User());
				
				zcount = userList.size();
				try {
					for(int i=0;i<userList.size();i++){
						if(Tools.checkMobileNumber(userList.get(i).getPHONE())){			//手机号格式不对就跳过
							if("1".equals(TYPE)){
								SmsUtil.sendSms1(userList.get(i).getPHONE(), CONTENT);		//调用发短信函数1
							}else{
								SmsUtil.sendSms2(userList.get(i).getPHONE(), CONTENT);		//调用发短信函数2
							}
							count++;
						}else{
							continue;
			...
			//if(strEM.length == 4){
				if("yes".endsWith(isAll)){
					try {
						...
						try {
							for(int i=0;i<userList.size();i++){
								if(Tools.checkEmail(userList.get(i).getEmail())){		//邮箱格式不对就跳过
//									mailSender.sendEmail(userList.get(i).getEmail(), TITLE, CONTENT, TYPE);//调用发送邮件函数
									count++;
								}else{
									continue;
								}
...
//								mailSender.sendEmail(arrTITLE[i], TITLE, CONTENT, TYPE);//调用发送邮件函数
								count++;
							}else{
								continue;
							}
