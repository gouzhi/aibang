...
<div id="page-content" class="clearfix">
  <div class="row-fluid">
	<div class="row-fluid">
			<!-- 检索  -->
			<form action="recharge/list" method="post" name="Form" id="Form">
			<table>
				<tr>
				    <td style="vertical-align:top;"> 
					 	<select  name="paymentId" id="paymentId"  style="vertical-align:top;width: 120px;">
							<option value="">&nbsp;全部类型</option>
							<c:forEach items="${payments}" var="item" varStatus="vs">
							  <option value="${item.id}" ${item.id eq query.paymentId?"selected='selected'":""}>${item.pname }</option>
							</c:forEach>
					  	</select>
					</td>
					<td style="vertical-align:top;"> 
						...
						<c:if test="${QX.cha == 1 }">
							<c:forEach items="${page.result}" var="var" varStatus="vs">
								<tr>
									...
									<td>${var.BALANCE}</td>
									<td>
									<%-- <c:if test="${var.TYPE==1}">网银</c:if>
									<c:if test="${var.TYPE==2}">快捷</c:if> --%>
									${var.P_NAME}
									</td>
									<td>
									 <c:forEach items="${statusTypes}" var="item" varStatus="vs">
										...
										<div class='hidden-phone visible-desktop btn-group'>
											...
											<c:if test="${QX.cha == 1 }">
											    <div class="hidden-phone visible-desktop btn-group">
												   ...
												   <a onclick="sysn(${var.ID});" title="查看" class="btn btn-mini btn-primary">同步</a>
										        </div>
										    </c:if>
										</div>
									</td>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		{
			top.mainFrame.showTabPage("充值详细",'${project_name}/recharge/detail/'+id);
		}
		//检索
		function search(){
			top.jzts();
