...
<div class="accountContent clear">
	...
	<div class="account-Right fl">
		...
		<div class="whiteR329">
			<div class="detailedTab"> <span onclick = "having()" class="action">持有中的投资</span> <span onclick = "over()">已结清的投资</span> <span onclick = "record()">申请记录</span> </div>
			<form  action="${project_name}/user/invest/investHaving" method="post">
				<div class="detailedList" style="display:block;">
					...
					<table class="detailedList-head">
						<tr>
							<th class="detSp1">投资名称</th>
							<th class="detSp2">年化收益率</th>
							<th class="detSp3">投资期限</th>
							<th class="detSp4">投资本金（元）</th>
							<th class="detSp5">预期收益（元）</th>
							<th class="detSp6">待收本息（元）</th>
							<th class="detSp7">投资结束日</th>
							<!--<span class="detSp6">预计下期收款额（元）</span>
                                        <span class="detSp7">操作</span>-->
						</tr>
						<c:choose>
							<c:when test="${not empty page.result }">
								<c:forEach items="${page.result }" var="item">
									<tr class="detailedList-con">
										<td class="detSp1" ><a href="user/invest/investHavingDetail?tId=${item.ID}">${item.B_NAME }</a></td>
										<td>${item.BORROW_APR }%</td>
										<td>${item.BORROW_PERIOD}${item.REPAY_TYPE==1?'月':'天'}</td>
										<td>${item.TENDER_ACCOUNT }</td>
										<td class="pinkFC328">${item.APPR}</td>
										<td class="pinkFC328">${item.APPR+item.TENDER_ACCOUNT }</td>
										<td><fmt:formatDate value="${item.RECOVER_TIME}" pattern="yyyy-MM-dd "/></td>
										<!--<span class="detSp6">280.00</span>
                                            <span class="detSp7"><a href="#">赎回</a><b>|</b><a href="#">投资</a></span>-->
									</tr>
								</c:forEach >
							</c:when>
...
function over(){
	location.href = "${project_name}/user/invest/investOver?b="+Math.random();
}

function record(){
	location.href = "${project_name}/user/invest/investRecord?b="+Math.random();
}
function having(){
	location.href = "${project_name}/user/invest/investHaving?b="+Math.random();
}
