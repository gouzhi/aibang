...
<script type="text/javascript">
	...
	function addrole(){
		...
		if(checkrole(roleid)){
			$("#addrole").after("<tr id='tr_"+roleid+"' class='info'><td><input type='hidden' id='"+roleid+"' name='roles["+roleid+"].id' value='"+roleid+"'/>"+roleName+"<a class='btn btn-mini btn-danger' title='删除' onclick='delrole("+roleid+");'>删除</a></td></tr>");
...
<body>
	<form action="user/editU.do" name="userForm" id="userForm" method="post">
		<input type="hidden" name="id" id="user_id" value="${pd.id }"/>
		<input type="hidden" name="typeId" id="typeId" value="${pd.typeId}"/>
		<input type="hidden" name="createTime" id="createTime" value="<fmt:formatDate value="${pd.createTime }"  type="both" />"/>
		<input type="hidden" name="delids" id="delids" value="" />
		<input type="hidden" name="isDel" id="isDel" value="${pd.isDel}"/>
		<div id="zhongxin">
		<table>
			...
			<c:if test="${!empty pd.roles}">
				<c:forEach items="${pd.roles}" var="rol">
					<tr id='tr_${rol.id}' class='info'>
						<td>
							<input type='hidden' name='roles[${rol.id}].id' id='${rol.id}' value='${rol.id}'/>
							${rol.roleName }
							<a class='btn btn-mini btn-danger' title='删除' onclick='delrole(${rol.id});'>
								删除
							</a>
						</td>
					</tr>
