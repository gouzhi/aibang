...
<div id="page-content" class="clearfix">
  <div class="row-fluid">
	<div class="row-fluid">
			<!-- 检索  -->
			<form action="refund/list" method="post" name="Form" id="Form">
						...
						<c:if test="${QX.cha == 1 }">
							<c:forEach items="${page.result}" var="var" varStatus="vs">
								<tr>
									...
									<td><fmt:formatDate value="${var.REPAY_TIME}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
									<td>${var.REPAY_ACCOUNT}</td>
									<td>
									<c:choose>
									  <c:when test="${ var.VERIFY_STATUS==5}">待还款</c:when>
									  <c:when test="${ var.VERIFY_STATUS==11}">未通过</c:when>
									  <c:when test="${ var.VERIFY_STATUS==8}">已还款</c:when>
									</c:choose>
									</td>
									<td style="width: 30px;" class="center">
										<div class='hidden-phone visible-desktop btn-group'>
											...
											<c:if test="${QX.audit == 1 and var.VERIFY_STATUS!=8}">
											    <div class="hidden-phone visible-desktop btn-group">
												   <a onclick="refundAudit('${var.ID}',${var.VERIFY_STATUS});" title="操作" class="btn btn-mini btn-purple">操作</a>
										        </div>
										    </c:if>
										     <c:if test="${QX.audit == 1 and var.VERIFY_STATUS==8 }">
											    <div class="hidden-phone visible-desktop btn-group">
	...
	<!-- PAGE CONTENT ENDS HERE -->
        ...
        //还款审核 3 还款待审,6 还款失败	
		function refundAudit(id,state)
		{
			top.jzts();
		 	var diag = new top.Dialog();
		 	diag.Drag=true;
			diag.Title ="操作";
			diag.URL = '${project_name}/refund/auditing.do?id='+id;
			diag.Width = 600;
			diag.Height = 270;
			...
			top.mainFrame.reloadPage(/refund\/list/);
			diag.close();
			};
			diag.show();
		}
