...
<form action="usersOpt/editU.do" name="userForm" id="userForm" method="post">
	...
	<div class="row-fluid">
        ...
        <div class="span4">
            <select class="span11" name="roleinfo" id="role_id" validate="{messages:{required:'请选择角色'}}">
	...
	<div class="row-fluid" id="addrole">
		...
		<c:if test="${!empty pd.hjsUser.roles}">
				<c:forEach items="${pd.hjsUser.roles}" var="rol">
				<div class='span2' id='div_${rol.id}'>
					<input type='hidden' name='hjsUser.roles[${rol.id}].id' id='${rol.id}' value='${rol.id}'/>
					${rol.roleName }
					<a class='btn btn-mini btn-danger' title='删除' onclick='delrole(${rol.id});'>删除</a>
				</div>
				</c:forEach>
			</c:if>	
	</div>
	<div class="row-fluid">
    	...
    	<div class="span10">
        	<textarea class="span11" name="remark" id="remark" placeholder="这里输入备注">${pd.remark }</textarea>
    	</div>
	</div>
    <div class="row-fluid">
...
function addrole(){
	...
	if(checkrole(roleid)){
		$("#addrole").append("<div class='span2' id='div_"+roleid+"'><input type='hidden' name='hjsUser.roles["+roleid+"].id' id='"+roleid+"' value='"+roleid+"'/>"+roleName+"<a class='btn btn-mini btn-danger' title='删除' onclick='delrole("+roleid+");'>删除</a></div>");
